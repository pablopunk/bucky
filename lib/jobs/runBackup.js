...
access_key_id = ${credentials.accessKeyId || credentials.accessKey}
secret_access_key = ${credentials.secretAccessKey || credentials.secretKey}
...
[${remoteName}]
type = b2
account = ${credentials.applicationKeyId || credentials.accountId}
key = ${credentials.applicationKey || credentials.key}
`;
        break;
      case 'storj':
        remoteName = 'storjremote';
        remoteConfig = `
[${remoteName}]
type = s3
provider = Other
access_key_id = ${credentials.accessKey || credentials.accessKeyId}
secret_access_key = ${credentials.secretKey || credentials.secretAccessKey}
...
`;
    ...
    // Define remote path
    const remotePath = `${remoteName}:${credentials.bucket}/${job.remote_path.replace(/^\/+/, '')}`;
    ...
    try {
      ...
      logger.info(`Running rclone with args: ${rcloneArgs.join(' ')}`);
      ...
      const updatedJob = db.prepare(`SELECT status FROM backup_jobs WHERE id = ?`).get(jobId);
    ...
    try {
      ...
      const job = db.prepare(`SELECT name FROM backup_jobs WHERE id = ?`).get(jobId);
      if (job) {
